...
import re
import logging
from ..database.sqlite import load_data
...
def make_decision(product_name: str, product_prices, product_reviews: list[dict]) -> dict:
    ...
    logger.info(f"Producing decision for product: {product_name}")
    results = analyze_reviews(product_reviews)
    # Get the dictionary keys from the new return format
    sentiment_counts = results["sentiment_distribution"]
    total_reviews = results["total_analyzed"]
